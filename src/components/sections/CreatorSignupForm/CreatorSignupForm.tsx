...
} from "@/schemas/CreatorRegisterSchema";
...
const CreatorSignupForm = () => {
  ...
  const methods = useForm<CreatorRegisterData>({
  ...
  const onSubmit = async (data: CreatorRegisterData) => {
    try {
      setIsLoading(true);
      ...
      const response = await API.registerCreator(formData);
  ...
  return (
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        {/* Phone Number Field */}
        <PhoneNumberInput<CreatorRegisterData>
          label="Phone number"
        />
