...
export const CourseContentPreview = ({ previewUrl }: Props) => {
    ...
    <div className="relative h-full w-full bg-creator-bg-400">
      ...
      <div className="absolute w-full bottom-2 left-2 flex items-center justify-between ps-6 py-2 pe-8">
        <button
          ...
          className="bg-creator-bg-400 bg-opacity-75 rounded-full p-1.5"
        >
          {isPlaying ? (
        ...
        <button
          onClick={handleMute}
          className="bg-creator-bg-400 bg-opacity-90 rounded-full p-1.5"
        >
          {isMuted ? (
